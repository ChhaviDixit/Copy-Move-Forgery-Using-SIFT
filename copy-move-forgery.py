...
def get_candidate_keypoints(D, w=16):
	...
	print("Number of Keypoints generated: ",len(candidates))
	return candidates
...
def assign_orientation(kps, octave, num_bins=36):
    ...
    for kp in kps:
        ...
        L = octave[...,s]
        histo = np.zeros(num_bins, dtype=np.float32)
        for oy in range(-w, w+1):
            for ox in range(-w, w+1):
                ...
                histo[bin] += weight
        # if counter==0:
        #     plt.hist(histo,num_bins)
        #     plt.show()
        #     counter+=1
        max_bin = np.argmax(histo)
        new_kps.append([kp[0], kp[1], kp[2], fit_parabola(histo, max_bin, bin_width)])
        max_val = np.max(histo)
        for binno, val in enumerate(histo):
            if binno == max_bin: continue
            if .8 * max_val <= val:
                new_kps.append([kp[0], kp[1], kp[2], fit_parabola(histo, binno, bin_width)])
...
if __name__ == '__main__':
    ...
    im=imread("OneDrive/Desktop/ip/image processing project/insul.jpg")
    plt.imshow(im)
    plt.show()
    s=3                                                 
    num_octave=4                                        
    ...
    t_c=0.03
    w=16
    im_a = convolve(rgb2gray(im), gaussian_filter(s0))
    R_th = (r_th+1)**2 / r_th

    gaussian_pyr = generate_gaussian_pyramid(im_a, num_octave, s, sigma)
    ...
    print("")
    print("Number of keypoints in each octave after Processing: ")
    for i in feats:
        print(len(i))
    print("Number of values in each feature point: ",len(feats[0][0]))
    print("Vector value of one feature point: ")
    print(feats[0][0])
    print("\n Sample values corresponding to each keypoint in keypoint pyramid")
    print(kp_pyr[0][0])
    ...
    for i in range(num_octave):
        ax[i].imshow(im)
        # to scale the feature points to image size
        scaled_kps = kp_pyr[i] * (2**i)
        ax[i].scatter(scaled_kps[:,0], scaled_kps[:,1], c='r', s=10)
    plt.show()
    ...
    for i in range(num_octave):
        ...
        for p in range(len(indices)):
			# print(k)
            if distances[p][1]<0.3:
                k=indices[p]
                start=(int(scaled_kps[k[0]][0]),int(scaled_kps[k[0]][1]))
                end=(int(scaled_kps[k[1]][0]),int(scaled_kps[k[1]][1]))
                image=cv2.line(image, start, end,(0,255,0))
        plt.imshow(image)
        plt.show()
			# print(start, end)
